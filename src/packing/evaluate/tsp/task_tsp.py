...
def evaluate(function_from_llm, dataset_struct, iter_limit_map):
    ...
    logging.info(f"[*] Loading optimal_objs_dict_{split}_drop_{drop_rate}_townsizes_100_200_perturbations_20_20_iterlimits_16_8.pkl")
    try:
        preamble = "."
        with open(f"{preamble}/dataset/optimal_objs_dict_{split}_drop_{drop_rate}_townsizes_100_200_perturbations_20_20_iterlimits_16_8.pkl", "rb") as f:
...
def evaluate_func(cfg, dataset_config, function_class):
    ...
    try:
        ...
        assert isinstance(avg_mean_obj, (float, int, np.float64, np.float32, np.float16, np.int64, np.int32, np.int16, np.int8))
    except Exception as e:
        tb_str = traceback.format_exc()
        function_class.fail_flag = 1
