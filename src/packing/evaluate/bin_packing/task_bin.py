...
# def evaluate(instances: dict, priority_func: Callable) -> float:
...
#     # has shape (num_items,).
#     #TODO: Here check that it is a valid packing
...
def evaluate_func(cfg, dataset_name, function_class):
	...
	#logging.info("Entered evaluate_func")
	...
	#logging.info("Executed imports and function")


	#TODO: Check that it is a valid packing (above)
	#neg_avg_num_bins = evaluate(datasets[dataset_name], priority_func)
	...
	for name in instances:
		...
		for item in items:
			...
			try:
				...
				assert np.array_equal(valid_bin_indices, valid_bin_indices_original), "Valid bin indices have been modified"
				...
				assert isinstance(best_bin, (int, np.int16, np.int32, np.int64)), f"Best_bin is not an integer but a {type(best_bin)}"
			except Exception as e:
				tb_str = traceback.format_exc()
				function_class.fail_flag = 1
	...
	#avg_num_bins = evaluate(datasets[dataset_name], priority_func)
	...
	#logging.info(f"About to return function_class, score achieved: {score}")
