...
def get_full_model_name(cfg):
    def get_name(name):
        if cfg.cluster == "rcp":
            folder_name = "../../../../shared/packing_models"
        elif cfg.cluster == "kuma":
            folder_name = "/scratch/surina/packing_models"
        if name == "llama3":
    ...
    if isinstance(cfg.model_name, (list, ListConfig)):
        return [get_name(name) for name in cfg.model_name]
    else:
        return get_name(cfg.model_name)
...
def initialize_single_model(
    ...
    # if load_finetuned:
    #     # Load adapter for inference. The adapter for training is loaded in the training script.
    #     logging.info(f"Using model adapter from {model_adapter_dir}")
    #     model = PeftModel.from_pretrained(model, model_adapter_dir)
    #     logging.info(f"Model.base_model.peft_config {model.base_model.peft_config}")
    #     logging.info("Merging model and adapter")
    #     model.merge_and_unload(progressbar=True, safe_merge=True)
    # else:
    #     logging.info("Not using an adapter")
...
def initialize_models(cfg, load_finetuned):
    ...
    if cfg.multiple_models:
        assert isinstance(cfg.model_name, (list, ListConfig))
        assert isinstance(cfg.full_model_name, (list, ListConfig))
        assert isinstance(cfg.gpu_nums, (list, ListConfig))
        assert len(cfg.model_name) == len(cfg.gpu_nums) == len(cfg.full_model_name)
...
def generate_on_gpu(chat, model, tokenizer, sampling_params, gpu_num, load_finetuned):
    # logging.info(f"{Accelerator().local_process_index}")
    tokenized_chat = tokenizer.apply_chat_template(
        chat, tokenize=True, add_generation_prompt=True, return_tensors="pt"
    ).to(
        f"cuda:{gpu_num}"
    )  # .to(f"cuda:{Accelerator().local_process_index}") #.to(f"cuda:{gpu_num}")
    len_input_token_ids = len(tokenized_chat[0])

    logging.info(f"Tokenization finished, generating on GPU {model.hf_device_map}...")
    model.eval()

    # Assert that the model is on the correct device
    # assert gpu_num == model.device.index

    # Check that the model has adapter loaded
    # if load_finetuned:
    #     logging.info(f"model.base_model.peft_config: {model.base_model.peft_config}")
    #     logging.info(f"Using an adapter")
    # else:
    #     # Verify that checking model.base_model.peft_config raises an error
    #     try:
    #         _ = model.base_model.peft_config
    #         logging.error("Peft_config should not be loaded")
    #     except:
    #         logging.info("Not using an adapter")
...
def generate_from_server(cfg, chat, flag_load_finetuned, server_port, num_outputs_per_prompt):
    # TODO: No need to apply_chat_template here, it should be done in the client. check if add_generation_prompt is taken care of by the server
    # TODO: pad token?
    ...
    # TODO: Check if vLLM output matches
...
def initialize_models_server(cfg, load_finetuned, use_vllm=False):
    if cfg.multiple_models:
        ...
        server_pids = []
        model_ids = []
        ports = [8080 + i for i in range(len(cfg.model_name))]
...
# relevant commands for killing all processes related to tgi
# ps aux | grep text-generation
# pgrep -f text-generation
# pkill -f text-generation
# pkill -9 -f text-generation
...
# Below is old code for multiprocess generation that is not working
# def single_process_generation(args):
#     (chat, model, tokenizer, sampling_params, gpu_num, load_finetuned, time_start) = args
#     (
#         extracted_functions,
#         extracted_imports,
#         outputs_text,
...
#     tasks = []
#     for model, tokenizer, sampling_params, gpu_num in zip(models_list, tokenizers_list, sampling_params_list, cfg.gpu_nums):
#         tasks.append((chat, model, tokenizer, sampling_params, gpu_num, load_finetuned, time_start))

#     with mp.Pool(processes=model_count) as pool:
#         results = pool.map(single_process_generation, tasks)
...
#         functions_collect[i] = extracted_functions
#         imports_collect[i] = extracted_imports
#         outputs_text_collect[i] = outputs_text
#         len_output_token_ids_collect[i] = len_output_token_ids
#         len_input_token_ids_collect[i] = len_input_token_ids
#         llm_generation_time_collect[i] = llm_generation_time
...
# Old code for cleaning up memory
# def clean_up_memory():
#     print(f"  --> Memory before cleanup: {torch.cuda.max_memory_allocated() // 1024 // 1024}MB")
...
#     #     pass
#     # try:
#     #     torch.distributed.destroy_model_parallel()
#     # except:
#     #     pass
...
#     for _ in range(3):
#         torch.cuda.empty_cache()
#         time.sleep(1)  # Give some time for CUDA to release memory

#     # Shutdown Ray (if used)
#     try:
#         import ray
...
#             ray.shutdown()
#     except ImportError:
#         pass  # Ray might not be used or installed

#     # Reset CUDA device (optional, use cautiously)
#     torch.cuda.reset_peak_memory_stats()
#     torch.cuda.reset_accumulated_memory_stats()
...
#     time.sleep(3)  # Increase sleep time to ensure cleanup
#     gc.collect()
#     torch.cuda.empty_cache()
#     logging.info(f"  --> Memory after cleanup: {torch.cuda.max_memory_allocated() // 1024 // 1024}MB")


# def clean_up_gpu_mem():
#     logging.info(
#         f"  --> Memory before cleanup: {torch.cuda.max_memory_allocated() // 1024 // 1024}MB"
#     )
#     for _ in range(2):
#         torch.cuda.empty_cache()
#         time.sleep(1)  # Give some time for CUDA to release memory
...
#     gc.collect()
#     logging.info(
#         f"  --> Memory after cleanup: {torch.cuda.max_memory_allocated() // 1024 // 1024}MB"
#     )
