...
class StoppableTask:
    def __init__(self, cfg, evaluate_func, input_struct, function_class, task_id, timeout):
        ...
        self.process = None
        # if cfg.task_flatpack:
        #     # We need to do this to make it compatible with spawn processes
        #     self.manager = multiprocessing.Manager()
        #     self.result_queue = self.manager.Queue()
        #     self._process_lock = self.manager.Lock()
        #     self._timer_lock = self.manager.Lock()
        #     # Shared state for signalling timeouts
        #     self._timeout_event = self.manager.Event()
        #     # This event ensures stop() only runs its shutdown steps once
        #     self._already_stopped = self.manager.Event()
        # else:
        ...
        self.mem_limit_bytes = int(
            cfg.mem_limit_gb * 1024 * 1024 * 1024)  # getattr(cfg, "memory_limit_bytes", 0.5 * 1024 * 1024 * 1024)
    ...
    def stop(self):
        ...
        # # Close and join queue explicitly
        # try:
        #     self.result_queue.close()
        #     self.result_queue.join_thread()
        # except Exception as e:
        #     logging.error(f"[Task {self.task_id}] Error closing queue: {e}")
