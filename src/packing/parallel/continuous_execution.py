...
def Producer(
    ...
    """
    # TODO: This for loop can probably be parallelized using multithreading or map
    try:
        for (
                chat,
                prompt_scores,
                ...
                try:
                    with StoppableTask(
                        ...
                        if not task.is_timed_out():
                            ...
                            if result_tuple is not None:
                                ...
                                if isinstance(result_class, FunctionClass):
                                    if result_class.fail_flag == 0:
                                        ...
                                        # Mark as failed
                                        assert result_class.score == cfg.failed_score
                                        assert result_class.true_score == cfg.failed_score
                                ...
                                # wandb.log(
                                #     {
                                #         "task_completed_no_result": worker_id,
                                #         "round_num": round_num,
                                #         "exit_code": exit_code if exit_code is not None else -999
                                #     }
                                # )
                        else:
                            ...
                            failed_function_class.score = cfg.failed_score
                            failed_function_class.true_score = cfg.failed_score
...
def consumers_finish_and_cleanup(
    ...
    # while not function_classes_to_eval.empty():
    #     logging.info(
    #         f"Waiting for the queue to empty. Current size: {function_classes_to_eval.qsize()}"
    #     )
    #     time.sleep(5)
    #     pass
    # logging.info("Queue is empty, closing it.")
    # function_classes_to_eval.close()
    # logging.info("Joining the queue thread.")
    # function_classes_to_eval.join_thread()
        ...
        for worker_id, p in enumerate(consumers):
            try:
                ...
                timeout_time = math.ceil(
                    (queue_size + 1) / (cfg.num_workers / 2)) * cfg.timeout_period + cfg.timeout_period
...
def evaluate_on_evalset(
    ...
    input_struct = generate_input(cfg, evalset)
